...
// Constantes globales de pentagonos y tamaño de pantalla de juego (fijas)
                               ...
                               int fIni, int cIni, int fFin, int cFin)
{
    ...
    while (!q.empty())
    {
        ...
        vector<pair<int, int>> vecinos = {
            {f - 1, c}, {f + 1, c}, (f % 2 == 0) ? pair<int, int>{f - 1, c - 1} : pair<int, int>{f + 1, c - 1}, (f % 2 == 0) ? pair<int, int>{f - 1, c + 1} : pair<int, int>{f + 1, c + 1}};
...
// Reconexión forzada: intenta abrir un camino hacia la meta si queda aislada la meta
...
// Verificar si hay una ruta desde el jugador hacia una celda
...
bool elegirNuevaMeta(vector<string> &mapa, int fJug, int cJug, int &fMeta, int &cMeta,
                     const map<pair<int, int>, Vector2f> &posCelda, CircleShape &meta)
{
...
// FUNCIÓN DE JUEGO: ejecución principal del nivel
float margenLateralIzq = 50.f;
float anchoPanelDerecho = 250.f;

bool ejecutarJuego(RenderWindow &window, const string &archivoMapa, Font &fuente)
{
    ifstream archivo(archivoMapa);
    if (!archivo)
    {
        tinyfd_messageBox("Error", ("No se pudo abrir:\n" + archivoMapa).c_str(), "ok", "error", 1);
        return false;
    }
    ...
    Text txtIndicacion("Haz click donde estara \nel jugador y luego la meta", fuente, 16);
    txtIndicacion.setFillColor(Color::Red);
    txtIndicacion.setPosition(xPanel, WINDOW_HEIGHT - 80);
    Clock relojIndicacion;
    bool mostrarIndicacion = true;
    ...
    auto aplicarCambiosAleatorios = [&]()
    {
        for (size_t i = 0; i < filas; ++i)
            for (size_t j = 0; j < mapa[i].size(); ++j)
            {
                if ((int)i == fJug && (int)j == cJug)
                    continue;
                if ((int)i == fMeta && (int)j == cMeta)
                    continue;
                if (mapaOriginal[i][j] == '.')
                    continue;
    ...
    auto moverJugador = [&](int nf, int nc)
    {
        ...
        if (bateria <= 0)
        {
            ...
            string msg = "¡Batería agotada!\n\n"
                         "Movimientos realizados: " +
                         to_string(movs) +
                         "\nTiempo: " + to_string((int)segundos) + " segundos\n\n"
                                                                   "¿Qué deseas hacer?\nSí: Intentar de nuevo\nNo: Salir del juego\nCancelar: Volver al menú";

            int resp = tinyfd_messageBox("Fin del juego", msg.c_str(), "yesnocancel", "warning", 1);
            if (resp == 1)
            {
                fJug = cJug = fMeta = cMeta = -1;
                mapa = mapaOriginal;
                movs = 0;
                bateria = BATERIA_MAX;
                relojJuego.restart();
                ruta.clear();
                mostrarRuta = autoMover = false;
            }
            else if (resp == 0)
            {
                volverMenuCarga = true;
            }
            else
            {
                window.close();
    ...
    while (window.isOpen() && !volverMenuCarga)
    {
        ...
        while (window.pollEvent(ev))
        {
            ...
            // R: mostrar ruta y activar movimiento automático
        ...
        for (size_t f = 0; f < filas; ++f)
        {
            ...
            for (size_t c = 0; c < mapa[f].size(); ++c)
            {
                if (mapaOriginal[f][c] == '.')
                    continue;
                Sprite sp;
                sp.setTexture(mapa[f][c] == 'P' ? txOn : txOff);
                float factor = (RADIO * 2) / sp.getTexture()->getSize().x;
                sp.setScale(factor, factor);
                sp.setOrigin(sp.getTexture()->getSize().x / 2.f, sp.getTexture()->getSize().y / 2.f);
        ...
        barraCarga.setFillColor(perc > 0.66 ? Color::Green : perc > 0.33 ? Color::Yellow
                                                                         : Color::Red);
        ...
        //Indicaciones de clic (jugador/meta)
        if (mostrarIndicacion && relojIndicacion.getElapsedTime().asSeconds() < 5.f)
        {
            window.draw(txtIndicacion);
        }
        else
        {
            mostrarIndicacion = false;
...
int main()
{
    ...
    bgMenu.setScale(float(WINDOW_WIDTH) / bgMenuTx.getSize().x, float(WINDOW_HEIGHT) / bgMenuTx.getSize().y);
    ...
    while (window.isOpen())
    {
        ...
        else if (estado == Estado::MENU_CONFIG)
        {
            ...
            // Dibujar
            window.draw(campoBat);
            window.draw(campoMeta);
            window.draw(lblBat);
            window.draw(lblMeta);
            window.draw(txtBat);
            window.draw(txtMeta);
